...
class UsuariosController {
  ...
  static login = async (req, res, next) => {
    try {
      ...
      if (!email || !password) {
        return res.json({ status: 400, mensaje: 'Email y contraseña son obligatorios.' });
      }
      ...
      // Estados de seguridad (nuevo flujo)
      // 1) Si la cuenta no está verificada por email
      if (Number(usuario.email_verificado) !== 1) {
        return res.json({ status: 401, mensaje: 'Debes verificar tu correo antes de iniciar sesión.' });
      }

      // 2) Si la cuenta no fue aprobada por admin
      if (usuario.estado !== 'aprobado') {
        if (usuario.estado === 'pendiente') {
          return res.json({ status: 401, mensaje: 'Tu cuenta está pendiente de aprobación por un administrador.' });
        }
        if (usuario.estado === 'rechazado') {
          return res.json({ status: 403, mensaje: 'Tu registro fue rechazado. Revisa tu correo.' });
        }
      }

      // 3) Compatibilidad con campo activo (0 pendiente, 1 activo, 2 deshabilitado)
      if (usuario.activo === 2) {
        return res.json({ status: 403, mensaje: 'Tu cuenta está deshabilitada.' });
      }
  ...
  static agregar = async (req, res, next) => {
    try {
      const { nombre, nombres, apellidos, email, telefono, password } = req.body || {};

      // Normalizar nombres/apellidos desde "nombre" si viene legacy
      const _nombres = (nombres || (nombre ? String(nombre).trim() : '')).trim();
      const _apellidos = apellidos ? String(apellidos).trim() : '';

      if (!_nombres || !email || !password) {
        return res.json({ status: 400, mensaje: 'Nombre(s), email y contraseña son obligatorios.' });
      }

      // Email único
      const yaExiste = await UsuariosRepo.buscarPorEmail(email);
      if (yaExiste) {
        return res.json({ status: 409, mensaje: 'El correo ya está registrado.' });
      }
      ...
      if (!isStrongPassword(password)) {
        return res.json({
          status: 422,
          mensaje: 'La contraseña debe tener mínimo 8 caracteres e incluir mayúscula, minúscula, número y símbolo.',
        });
      }
      ...
      try {
        if (UsuariosRolesModel?.agregar) {
          await UsuariosRolesModel.agregar({ id_usuario: insertId, id_rol: DEFAULT_ROL_ID });
        }
      } catch (e) {
        // silencioso si no existe esa tabla/flujo
        console.warn('Advertencia: UsuariosRolesModel.agregar no disponible o falló:', e?.message);
      }

      return res.json({
        status: 200,
        mensaje: 'Registro realizado con éxito. Tu cuenta está pendiente de aprobación por un administrador.',
        datos: { id_usuario: insertId, nombres: _nombres, apellidos: _apellidos, email, telefono: telefono || '', activo: 0 },
      });
    } catch (e) {
  ...
  static registrarUsuario = async (req, res) => {
    try {
      const {
        nombres, apellidos, fecha_nacimiento, email, documento, telefono,
        password, acepta_politicas, rol_nombre
      } = req.body;

      // Validaciones mínimas
      if (!nombres || !apellidos || !email || !documento || !password || acepta_politicas !== '1') {
        return res.status(400).json({ status: 400, mensaje: 'Datos obligatorios faltantes o TyC no aceptado.' });
      }
      if (!req.files?.cedula_frente?.[0] || !req.files?.cedula_reverso?.[0]) {
        return res.status(400).json({ status: 400, mensaje: 'Debe adjuntar frente y reverso de cédula.' });
      }
      ...
      const baseDir = path.join(process.cwd(), 'public', 'uploads', 'cedulas', String(usuarioId));
      const frenteTmp = req.files.cedula_frente[0].path;
      const reversoTmp = req.files.cedula_reverso[0].path;

      const frenteDest = path.join(baseDir, 'cedula_frente' + path.extname(frenteTmp));
      const reversoDest = path.join(baseDir, 'cedula_reverso' + path.extname(reversoTmp));

      await moveFile(frenteTmp, frenteDest);
      await moveFile(reversoTmp, reversoDest);

      // Guardar rutas (relativas)
      await UsuariosRepo.guardarDocumento(usuarioId, 'cedula_frente', frenteDest.replace(process.cwd(), ''));
      await UsuariosRepo.guardarDocumento(usuarioId, 'cedula_reverso', reversoDest.replace(process.cwd(), ''));
      ...
      return res.json({ status: 200, mensaje: 'Registro exitoso. Revisa tu email para verificar la cuenta.' });
    } catch (err) {
      console.error('Error registrarUsuario:', err);
      return res.status(500).json({ status: 500, mensaje: 'Error interno al registrar.' });
    }
  };
  ...
  static eliminar = async (req, res, next) => {
    try {
      const result = await UsuariosRepo.eliminar({ id_usuario: req.params.id_usuario });
      if (result.affectedRows > 0) {
        return res.json({ status: 200, datos: result });
      }
      return res.json({ status: 404, datos: {} });
